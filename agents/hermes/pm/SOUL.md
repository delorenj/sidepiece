...
`source = hermes://agent/sidepiece-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `sidepiece-dev` role).
...
Put `repo = sidepiece` in event data; never insert repo or agent
...
| --- | --- | --- |
| Bank | `agent-sidepiece-pm` | `sidepiece` |
...
```bash
hindsight memory retain sidepiece "sidepiece-pm: <fact>" --context <cat>
hindsight memory recall sidepiece "<question>"
```
