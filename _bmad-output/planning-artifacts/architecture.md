...
1. **Failure isolation under one authoritative taxonomy.** FR-3 owns the state list; FR-14 feeds it. Panes fail independently and never render a failure as empty or as a permanent spinner. *(Step 7: the taxonomy has **two** producers, not one — see P2 and D12. Six states were added in step 7, DS-23…DS-28.)*
...
7. **Staleness and cache invalidation.** FR-2's bounded cache and FR-12's refetch — the product's own loop invalidates its own reads. *(Step 7: FR-2's cache lives in the **Cockpit document**, not the Bridge — see D10. The Bridge's on-disk snapshot (D2) is a different thing with a different job.)*
...
**Note — corrected in step 7, because this sentence and step 4's sequence said different things.** The **repo scaffold** is the first implementation story: pnpm workspace, `tsconfig.base.json`, `biome.json` (including the P1 `project_id` ban), `mise` tasks. That is sequence step 0 below, and it exists because sequence step 1's `contract/` is a workspace package and cannot be created without it. `pnpm dlx wxt@latest init` and `shadcn@latest init` are **not** part of that story — they happen at sequence step 6 with the extension shell. Scaffolding a WXT app four steps before anything renders buys a loadable stub and a month of drift; PRD §10 already directed sequencing the extension behind a working Bridge.
...
D7 schema versioning · D8 warm-session registry shape · **D9 snapshot age is surfaced, never enforced** · D10 FR-2's cache is the Cockpit's, with a stated TTL
...
A version the binary does not recognise — a `user_version` *ahead* of the migrations this build carries, which is what a rollback produces — is **`DS-25`**, allocated in step 7. It is a rendered state, not a health-endpoint footnote: the Bridge starts, resolution and Tickets are storeless reads and stay live, and **Chat is gated**, because Turn history and Turn state are the store. Saying "a named FR-14 health state" and stopping was not enough — P2 gives that phrase a specific meaning (a `DsCode`, a row, a wording, a pane gate), and four places in this document used it as a way of not deciding.

**D10 — FR-2's cache is the Cockpit's, and it has a TTL.** *(Rewritten in step 7. The original put this cache on the Bridge, as `registry/cache.ts`. That was wrong on three counts and the third is decisive.)*

PRD §5 says "two Chrome windows mean two Cockpit documents with **independent caches** and independent resolution state." `EXPERIENCE.md`'s Chrome-restart row lists "the resolution cache's in-memory copy" under **Not preserved**, which is only true of a document that dies. Its Reopen rule 2 paints the identity header **from cache** and calls that "the difference between a reopen that feels instant and one that feels like a load" — a latency rationale the original D10 explicitly denied. And FR-2's own bound requires invalidation "when Bridge health transitions from unreachable to reachable," which is a transition **only the client can observe**: a Bridge that was unreachable was not running, and a process cannot notice its own absence. A cache placed on the Bridge cannot implement the one invalidation trigger its requirement names.

So:

- **The FR-2 resolution cache lives in the Cockpit document, in memory, one per window.** `lib/cache.ts`. It dies with the document, which is correct — the platform already guarantees the bound the PRD wants.
- **TTL: 5 minutes.** `[ASSUMPTION: five minutes. The registry is nineteen rarely-changing Projects measured at 2.4ms p50, so the TTL is not protecting a slow read — it is the hard ceiling FR-2 demands so that "cached for the panel session" is unreachable and FR-3's stale states are. Five minutes is long enough that a dip-out-and-back reopen (SM-C1's whole shape) is served from cache, and short enough that a Project renamed while he is away costs one stale header for one reopen, marked as-of the whole time. Tune it against SM-C1 if reopens start feeling like loads.]`
- **Invalidated on:** TTL expiry; an explicit re-resolve from any FR-3 state (the `{components.reResolveControl}` bypasses the cache, per `EXPERIENCE.md`); a Bridge health transition from unreachable to reachable, which `lib/bridge.ts` observes and which is what makes FR-15's "a Bridge restart does not require reloading the extension" true in practice; and **the generation advancing for that pjid** (D11) — the Bridge tells the client its current generation on every answer, so the client invalidates on *learning* an advance rather than on producing one. The original wording had the cached resolution invalidated by the stamp it had itself produced, which is circular and unimplementable.
- **It caches exactly one thing: the Project Record, for the identity header.** `EXPERIENCE.md`: "**The cache serves the header and nothing else** — no Ticket list, no Turn, ever." Every cached read renders with its as-of marker until the Bridge confirms.
- **Its relationship to the D2 snapshot, in one sentence:** the Cockpit's cache is a *client-side copy of a good answer*, read first and always marked with its age; the Bridge's `registry/snapshot.ts` is a *server-side copy of the upstream*, read only when the pjangler Registry does not answer and always marked with its age — invalidating the cache never deletes the snapshot, and serving from the snapshot never populates the cache as fresh.
- **`registry/cache.ts` is deleted from the Bridge.** There is nothing left for it to hold: the registry payload the Bridge fetches is one 33KB object it indexes in memory anyway, and its durable copy is the snapshot.
...
- The rule and its corpus live in **`contract/src/classify.ts`** — it crosses the boundary, so P9's MUST already required it to. One definition, imported by both.
...
- **The Cockpit document, in memory (authority for nothing, and it dies on collapse):** **FR-2's explicitly bounded resolution cache** (D10). *(Added in step 7. Rule 4 grants the Cockpit this as its one exception — "the Cockpit caches none of it beyond FR-2's explicitly bounded resolution cache" — and D6 did not carry the exception across, which is how the cache ended up on the Bridge.)* It is listed as its own tier rather than folded into `chrome.storage.local` because it must **not** survive the document: PRD §5 wants one cache per window and `EXPERIENCE.md` lists it under *Not preserved* across a Chrome restart. Writing it to `chrome.storage.local` would make it shared and durable, which is the opposite of both.
...
- **Matching is case-sensitive on the attribute** (HTML attribute *names* are case-insensitive, so `name="PJID"` also matches; the *value* is not normalised, ever). The pjid is author-controlled text and pjangler is the authority on what it means — a Bridge that lowercases it resolves a different row.
...
0. **Monorepo scaffold** — pnpm workspace, `tsconfig.base.json`, `biome.json` including the P1 `project_id` ban, `mise` dev/build/deploy tasks. *(Added in step 7. Step 1 below is a workspace package and cannot exist without this; step 3's own step-3 note said project initialization was "the first implementation story" and meant this, not WXT.)*
...
2. **`contract/`** — the twenty-eight-state taxonomy (DS-1…DS-28, split by producer per P2), Project Record incl. `generation` (D11), `CONTRACT_VERSION` (D12), Turn envelope, the classification rule and its corpus (D13), correlation id. Everything else imports it.
...
`[NOTE FOR PM: two prerequisites now sit off this repo's path, not one. D4's Bloodbank gateway fix is the first and already carried. The second is `agents/hermes/pm/role.yaml`, whose two illegal subscriptions are recorded in P3 — **not** as an FR-8 blocker (see the correction there), but as a live defect in the manifest of the very PM this product targets, which will be discovered by whoever builds FR-7 and will cost an hour of the wrong debugging if it is not fixed first. Carry it as a small prerequisite story, not as a blocker.]`
...
- **D10 → D11 → SM-3.** *(Added in step 7.)* The cache's placement, the generation's definition and the metric are one mechanism. Moving the cache client-side is what lets the generation invalidate it on *learning* an advance; making the generation content-addressed rather than a request counter is what stops it refusing PRD §5's legitimately-independent second window. Change either and SM-3's enforcement changes with it.
...
### P2 — Every failure carries a typed code. There are no ad-hoc error strings.
...
│   │       │                         #   Refusal, SubscriptionState      [P2]
│   │       ├── project.ts            # Project, ProjectRecord incl. generation,
│   │       │                         #   pjid, ticketProvider       [P1][D11][D21]
...
│       │                             #   auto-detect it; manifest surface [P9][D16]
...
│           │   ├── cache.ts          # FR-2's bounded cache: in-memory, per
│           │   │                     #   document, 5-min TTL              [D10]
...
- **No `registry/cache.ts` on the Bridge.** It was in this tree until step 7. FR-2's cache is the Cockpit's (D10); the Bridge's durable copy of the registry is the D2 snapshot and it is a different thing with a different job.
...
| **Bridge ↔ upstreams** | Process/network to seven services | One adapter folder per upstream; foreign names die at the adapter `[P1][P4]` |
...
| FR-2 | Resolve to a Project Record, bounded cache | `registry/client.ts`, `registry/generation.ts` | `lib/bridge.ts`, **`lib/cache.ts`** `[D10]` |
...
| FR-6 | Classify a Turn, visibly, overridably | `contract/src/classify.ts` (the rule), `turns/dispatch.ts` (authoritative re-check at send) `[D13]` | `cockpit/chat/` composer, evaluating the same rule on a typing pause |
...
- **Correlation** — minted as **`correlationId`** in `turns/dispatch.ts`, serialised to Bloodbank's **`correlationid`** only inside `bloodbank/adapter.ts`, matched back as `correlationId` in `bloodbank/outcomes.ts` and `turns/reconcile.ts`. *(Corrected in step 7: this bullet said `correlationid` keeps Bloodbank's spelling "throughout" and cited P4 as the authority, while P4's MUST says the opposite — "foreign names never travel inward past that adapter", and Sidepiece's own surfaces are camelCase. The id is minted inside `turns/dispatch.ts`, which is inward of the adapter. P8's log-key exception stays where it is, in P8.)*
...
| Bloodbank | `bloodbank/adapter.ts` (publish), `bloodbank/outcomes.ts` (consume) | NATS: `bb emit` out, durable consumer in `[P3][D19]` |
...
*(Corrected in step 7: the cache was drawn on the Bridge's side, between the registry and the snapshot. It is the Cockpit's, it is read before the network call rather than behind it, and the Bloodbank leg is bidirectional — publish out, outcomes back.)*
