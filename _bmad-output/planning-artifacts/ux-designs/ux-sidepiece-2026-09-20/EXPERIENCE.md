...
3. **One PRD literal did not fit, and this spine does not let it shrink.** The mock's own
   ...
   **Closed 2026-09-22, and closed by measurement rather than by concession.** `DESIGN.md`
   ...
   the other three escape hatches was needed and none was taken — no abbreviation, no smaller type,
...
continuity and display acceleration, with no authority: an unsent draft, which pane was last
open, which group was collapsed, `[v2]`'s pre-discharge annotations, and — `[RULED 2026-09-22
— Jarad]` — FR-2's bounded resolution cache, keyed by pjid (`architecture.md` D10, whose
MUST is that nothing here may be the only source of a rendered fact and anything rendered
from it carries its age). Rationale, in three parts. First, the
...
| **Window switch / multi-window** | Chrome's side panel is per-window: two windows are two Cockpit documents with **independent resolution state over a shared per-pjid resolution cache** *(amended 2026-09-22: `architecture.md` D10 places FR-2's cache in `chrome.storage.local`, so the store behind the identity header is shared across windows. Each panel still holds its own render, its own `(pjid, generation)` frame and its own SSE subscription — the independence that mattered survives; the duplicated store did not. Sharing is the safer direction here: two windows can no longer diverge, and a whole-entry replace cannot clobber.)*. v1 does not synchronize them (§5) | Two Cockpits may legitimately disagree, and **no sync indicator is rendered** — inventing one would imply a guarantee v1 does not make. FR-11's append-per-Turn rule keeps them from clobbering each other. The pane-selection preference is a single global key, so a switch in one window follows to the next open of the other; accepted `[ASSUMPTION: not worth a per-window key for a two-item toggle.]` |
...
| **Chrome restart** | **Preserved:** Turn history, because the Bridge holds it keyed by pjid and Turn id (FR-11, FR-15); drafts, pane selection and group-collapse state, because Rule 4 puts them in `chrome.storage.local`; and **the resolution cache** `[RULED 2026-09-22 — Jarad: FR-2's cache lives in `chrome.storage.local` keyed by pjid, not in the panel document (`architecture.md` D10). It is preserved across a restart, carrying the same qualifier drafts and pane selection do — preserved, and still only good for five minutes.]`. **Not preserved:** anything in the panel document, the SSE subscription. **Unknown:** whether Chrome reopens the side panel at all, and whether it restores the resized width | `[ASSUMPTION: assume the Cockpit is closed after a restart and that the first open is a full cold start — though the header's first paint may still be served from the resolution cache, marked as-of, when the entry is inside its five-minute TTL. Designing for a restored panel *document* would be designing for behavior nothing in the sweep confirms.]` |
