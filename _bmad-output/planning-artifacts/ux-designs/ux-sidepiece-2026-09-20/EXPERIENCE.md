...
naming the platform object rather than the product.
...
writing. When key-screen mocks are rendered at Finalize they are linked inline from the
...
| **Extension icon** | The `"this tab is resolvable"` signal (PRD §5) — FR-3's only always-on affordance | Always present, in browser chrome, on every tab | Chrome owns the frame; Sidepiece owns the glyph and the title |
...
drawn or accumulated is written outside the Cockpit before it is worth anything.
...
**The extension icon owns** exactly one bit: is this tab resolvable. **It must never own** a
Project name, a Ticket count, an unread count, or a menu. A badge that counts inbound work
is a badge that pulls, and SM-C1 counts pulling as failure. The icon exists because Chrome
forbids auto-open (PRD §5, P18) — its only job is to make the click informed.
...
- **~320px is a hard floor**, current as of Chrome 149, hard-coded with no flag and no
  ...
  handed.
- **The Cockpit cannot open itself and cannot close itself.** `sidePanel.open()` requires a
  genuine user gesture and must be the first synchronous call in the handler; an `await`
  before it silently no-ops (PRD §5). `sidePanel.close()` and `toggle()` do not exist —
  they are a standing feature request (w3c/webextensions#521). **The product cannot end a
  session; it can only fail to extend one.** Every "dip out" in SM-C1 is a gesture Jarad
  makes, which means the design's entire contribution to that metric is *not giving him a
  reason to stay*.
...
| Overlay `[v2]` | `{colors.overlay.outline}` · `{colors.overlay.scrim}` | The hover outline drawn over a page whose own colours are unknown; both must read against an arbitrary background |
...
That fork is Jarad's and it has been open since the abandoned 2026-08-25 DeloHQ run.
...
is a detail view, so the header cannot be one.
...
chrome.storage.local; rationale — a Board with eight states cannot show eight expanded
groups in 320px, and the default entry state is the group UJ-1 writes into.]` Inside the
Chat pane, the Turn list renders the most recent Turns and loads older on demand rather
than rendering the whole history on open `[ASSUMPTION: cold-start cost plus SM-C1; FR-11
requires history be restored, not that all of it be rendered at once.]` Inside the composer,
the description field on the Ticket create box and the context detail on the Turn composer
...
| Copy the clone path into a terminal | FR-4 | Clone path line, selectable | UJ-3 | yes |
...
| Tail Project-scoped events live `[v2]` | §9 | — | — | not v1; would take the fourth switch slot |
| See whether an Agent is already working this repo `[v2]` | §9 | — | — | not v1; would take the fourth switch slot |
...
icon opens the Cockpit in one click, and v2's recent-Projects list must find another home —
a switch slot or a context-menu item on the action — not a popup. Recorded here so v2 does
not spend the icon click without noticing.
...
cheap, quiet and non-accusatory — it is the wallpaper, not an error.
...
| Attachment on a Ticket | **Leading slot in the Ticket create box's submit row** | Same | Same |
| Batch create — one Ticket per annotation | **Secondary slot beside the create box's primary submit** | A submit row that holds a primary and one secondary control | The secondary control |
...
| Annotation count on the extension icon | **The icon's badge surface** | Nothing — the icon renders no badge in v1 | The badge |
...
|---|---|---|
| FR-4 pins three fields permanently | Permanent chrome is the definition of density | These three are the **only** permanently pinned data in the product, they are three lines, and SM-3 is unobservable without them (FR-4 says so). Not a precedent: nothing else gets pinned by citing this |
...
delete on each row, and one discharge control. **It empties on discharge.** It exists only
while an undischarged batch exists on the current page; with nothing in it, the third switch
slot is not rendered.

That is the difference from Vercel's Inbox and Figma's sidebar: those are where feedback
*lives*. This is where feedback *waits*, for as long as one browsing session. Nothing about
it rewards returning to it.

### What "good" looks like, stated so it can be checked

A healthy Sidepiece session is: icon says resolvable → one click → act → gone, inside a
minute. The design's contribution is negative — it does not give a reason to stay. Two
concrete consequences, both load-bearing:
...
| `"this tab is resolvable"` | Extension icon title (§5) | Exact. The icon's only words |
...
3. **Name the fix when there is one, and let the Bridge write it.** A remedy command is
   ...
   command be named, not that the Cockpit author it.]`
...
8. **Glossary or nothing.** Never "sidebar" for Cockpit, "message" for Turn, "issue" or
   "bug" for Ticket, "bot" or "assistant" for PM, "server" for Bridge, "project" lowercase
   for anything that is not a Project.
...
|---|---|---|---|
| `{components.identityHeader}` | Renders repo name, clone path, Board identifier, health marker, degraded-connection indicator. Present whenever a Project is resolved, without a menu or detail view (FR-4) | Nothing of its own. Re-derived from the Project Record on every resolution | Never renders a half-Project. Identity changes **in the same frame** as the body and the switch — one atomic render keyed by `(pjid, generation)`, never one pane on a Project the others left (FR-4, P11). On total outage the header is replaced by the shared notice, not left showing a Project we can no longer vouch for |
| Clone path line (inside the header) | Selectable as text so it can be copied into a terminal (FR-4) | — | Wraps; never truncates, never ellipsises. Carries a copy control as well, because a mouse-drag selection inside a 320px column is a poor primary. When the path is missing on disk (DS-9) it still renders, marked, because the path is the thing he needs to go look at |
| `{components.copyControl}` | Copies a complete machine string — clone path, command string, correlation identifier — to the clipboard | — | Confirms by swapping its own label for one beat, not by a toast. If the clipboard write throws, it says so in place and the text remains selectable, which was always the primary path (FR-4) |
| `{components.degradedConnectionIndicator}` | The §5 **degraded-connection indicator**: renders in the header while the Tailnet is on a DERP relay | — | It is a fact, not a fault, and it renders at `{colors.state.degraded}` rather than `{colors.state.failed}`. While it is present, every budget-derived deadline in State Patterns is suspended — §5 says budgets are "explicitly **not** guaranteed" on a relay, so converting to a timeout state would be reporting a failure the product was told not to promise against |
| `{components.paneSwitch}` | Selects which pane the body shows. Two items in v1, third slot reserved `[v2]` | Selected pane, per Project, in `chrome.storage.local` | Each item carries its own status marker, so a pane that is failing says so **while hidden**. This is how "every pane fails independently and says why" (§5) survives a one-pane-at-a-time body |
...
| `{components.stateNotice}` | The one block every degraded state renders into: a headline sentence, an optional detail line, an optional command string, and a recovery control | — | It **is** the failure path. It never renders as an empty region and never as a spinner (P1, P2). It always contains at least one control |
| `{components.reResolveControl}` | Re-runs detection and resolution for the active tab, bypassing the FR-2 cache | — | Present in **every** state notice (FR-2, FR-3), including ones it cannot fix — a Plane outage does not resolve by re-resolving, but the operator does not have to know that to reach for it. While running it disables and says so; when it returns the same state it says the state again rather than flashing |
| `{components.turnComposer}` | Collects a Turn. Holds the classification control, the context chip, and the reserved attachment slot `[v2]` | Draft text debounced to `chrome.storage.local` keyed by pjid; restored on open | Disabled with a distinct reason in both degraded Agent states (FR-5); the draft is kept while disabled. On send failure the text stays in the box. If storage throws or returns empty, the box renders empty and works |
| `{components.classificationControl}` | Shows the Turn's classification **before commit** and flips it in one control (FR-6) | Override lives for this Turn only and is discarded on send (FR-6, P14) | Renders the glossary term verbatim — **Streamed Exchange** / **Dispatched Command** — plus one plain line of consequence. It is never absent: an unclassifiable Turn classifies as Dispatched Command, per FR-6's stated bias, and says so rather than showing nothing |
...
| `{components.turnCard}` | Renders one Turn. Two visibly distinct variants: Streamed Exchange and Dispatched Command (FR-8) | Turn state lives in the **Bridge**, keyed by pjid and Turn id (FR-15). The card holds nothing | Streamed: a dead stream renders as failed with the partial text kept (FR-7). Dispatched: rejection is surfaced with its reason (FR-8); no outcome inside the window renders as unknown (FR-9); a completed outcome renders its result content, or says the gateway returned none (see IA note (a)). Never a bare checkmark |
...
| `{components.ticketGroupHeader}` | One per Board state, in the Board's own state order (FR-12, P7). Collapses to name plus count | Collapse state per Project in `chrome.storage.local` | A group with zero Tickets renders with a zero count rather than disappearing — the Board's shape includes its empty columns |
| `{components.ticketRow}` | Human key, title, state (FR-12). Opens the Plane URL in a new tab | — | A Ticket missing a field renders the fields it has and marks the gap; it is never dropped from the list |
| `{components.ticketCreateBox}` | Creates with a title alone. Description and target state optional; omitted state uses the Board's default entry state (FR-13). Submit names the Board it will write to | Draft debounced to `chrome.storage.local` keyed by pjid | On failure the entered text is preserved and the error is shown (FR-13). With no Board it is **disabled with the reason, not failing on submit** (FR-12, P19). A stale generation is refused before the network call, and says so (§5) |
| `{components.refetchControl}` | User-initiated Board refetch (FR-12) | — | While running the current list stays on screen — a refetch never blanks the pane. On failure the previous list stays and the failure is stated above it |
| `{components.commandString}` | Renders a remedy command the Bridge returned: mono, selectable, complete, with a copy control | — | If the Bridge returned no command, the notice omits the block entirely rather than rendering an empty one or inventing text |
...
| `{components.annotationRow}` `[v2]` | One row in the Annotations pane. Hovering it outlines its element in the page | — | Keeps its text and captured context when the anchor is lost; stays dischargeable |
...
| `{components.iconBadge}` `[v2]` | Count of undischarged annotations on the current tab | — | Absent in v1. Absent whenever the count is zero |

---

## State Patterns

The heart of this document. Three rules govern everything below.
...
- When the deadline passes, the in-flight state **becomes a timeout state with a retry
  control**. It does not keep spinning (P5).
...
six; §5's relay fallback, §5's generation refusal and the dev-reload case are real and
appear nowhere. The true count is **sixteen**. Each is separately worded, as FR-3 requires.

| # | Trigger | Panes gated | Wording | Recovery | Resolves when |
|---|---|---|---|---|---|
| **DS-1** | Active tab declares no pjid (FR-3 ①) | All — body shows the notice; no header | `This page declares no pjid.` / `Nothing to resolve. Sidepiece never guesses a Project from a URL.` | Re-resolve | A tab that declares one becomes active, or this page starts declaring one (FR-1 re-detects without a navigation) |
| **DS-2** | pjid declared, absent from the Registry (FR-3 ②) | All — body; no header | `` `<pjid>` — declared but unknown. `` / `The page declares this pjid. The Registry has no Project under it.` | Re-resolve | The Registry gains the Project, or the page declares a different pjid |
| **DS-3** | Bridge unreachable, laptop off the Tailnet (FR-3 ③, distinguishable case) | **Total** — one shared notice | `The laptop is off the tailnet.` / `` `burro-salmon.ts.net` isn't up on this machine, so the Bridge can't be reached. `` | Re-resolve | Tailnet returns; the FR-2 cache is invalidated on the unreachable→reachable transition |
| **DS-4** | Bridge unreachable, Tailnet up, host not answering (FR-3 ③, distinguishable case) | **Total** | `` `big-chungus` isn't answering. `` / `The tailnet is up. Nothing is listening at the Bridge.` | Re-resolve | Host or Bridge returns |
| **DS-5** | Bridge unreachable, cause not distinguishable | **Total** | `Can't reach the Bridge.` / `The tailnet looks up, but Sidepiece can't tell whether the host is down or the Bridge is stopped.` | Re-resolve | Either of the above becomes true and DS-3/DS-4 replaces this |
| **DS-6** | Bridge up, pjangler Registry **service not running** (FR-14) | **Total** — §6 marks Registry failure total | `The pjangler Registry service isn't running.` + command | Re-resolve | Service starts |
| **DS-7** | Bridge up, Registry **returned an error** (FR-14 — "the former has a different fix") | **Total** | `The Registry answered with an error.` / `<the error, as the Bridge reported it>` | Re-resolve | Registry answers cleanly |
| **DS-8** | Bridge up, a credential did not resolve from `DeLoSecrets` (FR-16 → FR-14) | Whichever dependency it feeds; named | `` The Bridge started without `<credential>`. `<dependency>` can't be trusted. `` | Re-resolve | Credential resolves at the next Bridge start or per-request retry |
| **DS-9** | Registry readable, clone path missing on disk (FR-3 ⑤) | None — informational; header renders and marks the path | `` `<path>` isn't on disk. `` / `The Registry has this Project. big-chungus doesn't have the clone.` | Re-resolve, plus the path stays selectable | Clone appears. Sidepiece never clones it (§4.1 Out of Scope) |
| **DS-10** | An Agent binding whose `role_dir` does not exist (FR-3 ⑥) | Chat | `` `<agent>` is bound to `<role_dir>`, which doesn't exist. `` | Re-resolve | The directory exists, or the binding changes. Occurs today: `.project.json` binds `sidepiece-scrum-master` to a path not in this repo |
| **DS-11** | No PM declared (FR-5 state 1) | Chat only — **Tickets stays fully live** (FR-5, P20) | `No PM is declared for <repo>.` + command | Re-resolve; the command is copyable and **is not run by v1** | A PM is declared in the Registry |
| **DS-12** | PM declared, absent from the Hermes fleet registry (FR-5 state 2, cause a) | Chat only | `` `<pm>` is declared but isn't in the Hermes fleet registry. `` | Re-resolve | The Agent is registered. Occurs today: `sidepiece-pm` |
| **DS-13** | PM declared and registered, not answering (FR-5 state 2, cause b; §6 "agent-unreachable") | Chat only | `` `<pm>` is registered but isn't answering. `` | Re-resolve | The gateway answers |
| **DS-14** | Project has no Board binding (FR-12) | Tickets — list unavailable, **create disabled not failing** (P19); Chat fully usable | `<repo> has no Board.` / `Nothing to list, and nothing to create against.` + command | Re-resolve | A Board is bound. Occurs today for four of nineteen Projects. The check is for a **truthy** identifier — the field is an empty string, never null (FR-12) |
| **DS-15** | Tailnet fell back to a DERP relay (§5) — the **degraded-connection indicator** | None; marks the header, and every budget-derived deadline is suspended | `Relayed connection. Timings below are not the usual ones.` | None; it is a fact, not a fault. Re-resolve is still present in the header's overflow | Direct path returns |
| **DS-16** | Extension reloaded; this tab's content script context is invalidated (addendum §C) | Detection, therefore all | `Sidepiece was reloaded. Reload this tab to reconnect.` | Re-resolve, which will fail until the tab reloads, and says so | The tab reloads |
...
- **Local Network Access denial.** Chrome's prompt reads "Look for and connect to any device
  ...
  the failure is distinguishable in practice, it earns DS-17 with its own wording naming the
  permission. Prior has moved: CGNAT `100.64.0.0/10` is explicitly `local` in the WICG spec
  and Chrome states extensions with host permissions are unaffected, so the expected result
  is no prompt at all.]`
...
| **Resolved from cache** | Header | The header renders immediately from the bounded FR-2 cache and carries an as-of marker until the Bridge confirms. The cache expires on its TTL and is invalidated on an unreachable→reachable transition (FR-2, P25) |
| **Reading the Board** | Tickets pane | `Reading the Board.` Deadline ≤2s p95. On expiry → `The Board read didn't come back inside 2s.` + retry (FR-12) |
| **Board empty** | Tickets pane | `This Board is empty.` After a 200 only. Distinct from DS-14 and from a failed fetch (FR-12) |
| **Refetching** | Tickets pane | The existing list stays on screen with an in-progress marker on the refetch control. A refetch never blanks the pane |
...
| **Dispatch rejected** | Turn card | The rejection and its reason, e.g. `Bloodbank refused the command: the subject is not five tokens.` Never silent success (FR-8) |
| **Dispatched — unknown** | Turn card | `No outcome after <window>. Status unknown.` Shown as unknown, **not** as success (FR-9, P9). The window is long enough that ordinary agent work does not routinely trip it |
...
| **Ticket created** | Tickets pane + create box | The row appears at the top of its group without a manual refresh (FR-13). Title clears, focus stays in the title field, description collapses. Nothing navigates |
...
| **Generation stale** | Wherever the mutation was attempted | `The Project changed while that was in flight. Nothing was written.` Refused by both the Cockpit and the Bridge (§5). A confidently wrong Ticket is worse than a failed one |
...
Cockpit is closed. It has exactly three, and it never has a fourth.
...
The icon **never** renders a count, a Project name, or a dot that means "something happened."
Clicking it always opens the Cockpit, in every state including the two degraded ones — the
signal makes the click informed, it does not gate it, and the degraded states are exactly
where the operator most needs the Cockpit's sentence about why.
...
2. **The identity header paints next, from cache if the cache is valid for that pjid.**
   ...
   expired, the header renders the pjid and one `Resolving` line.
...
4. **No pane waits on another.** The composer is live before the Turn history arrives. The
   create box is live before the Board arrives, and disables only if the Board read returns
   DS-14.
5. **Every progress line carries its budget as a deadline** and converts to a timeout state
   on expiry, per Rule 1.
...
and pane selection this spine puts in `chrome.storage.local`. `[ASSUMPTION: onClosed is
...
label. The override lives for that Turn only (P14). The bias toward Dispatched Command is
...
- A known Chromium bug throws the gesture error on the *second* open after a manual close
  ...
  the icon's title rather than only to the console.]`
- **The Cockpit cannot close itself.** There is no `close()` or `toggle()`
  (w3c/webextensions#521). No control in the Cockpit ever claims to close it.
- **Collapse is the same event as close** for the document. Anything unsaved at collapse is
  gone, which is why every draft in this spine is debounced to `chrome.storage.local` rather
  than held in the document.
...
|---|---|---|---|---|---|
| 1 | Open the Cockpit | `_execute_action` | `Alt+Shift+S` | No | Reserved name; triggers the action, which with `setPanelBehavior({openPanelOnActionClick:true})` opens the Cockpit. Requires the action stay popup-less — see IA note (c) |
| 2 | File a Ticket | `focus-ticket-title` | `Alt+Shift+N` | No | Opens the Cockpit if closed, switches to Tickets, puts the caret in the title field. A `commands` keypress is a permitted gesture, so the open call is legal from this handler |
...
| `Alt+M` | Composer focused | Flip the classification `[ASSUMPTION: the flip needs a key because FR-6 makes it a per-Turn decision and reaching a button by Tab on every Turn is a tax. The control is also a button, so the key is an accelerator, not the only path.]` |
...
| Anywhere in the Cockpit | Nothing else | Close the Cockpit — it cannot |
...
- **Drag, in exactly one place: `[v2]` the armed select tool**, where click yields an
  element-anchored annotation with no image and drag yields an image-bearing one. One armed
  mode, two payloads, no mode switch — this is Vercel Toolbar's shipped design, verbatim:
  "Click and drag while in commenting mode to automatically screenshot a portion of the page
  and start a comment with it attached" (`research-annotation-prior-art.md` §2, P8).
...
- The Turn list is bottom-anchored while streaming and **stops following the moment the
  ...
  you while you are reading it is the fastest way to make a panel feel hostile.
...
- **The clone path is selectable text** (FR-4) and is the only field the PRD names as such.
  ...
  copyable and copies wrong. It also carries a copy control, because a drag-selection in a
  320px column across a wrapped line is not a primary interaction.
...
- **Live regions, used sparingly and correctly.** Streaming text goes into a `polite` live
  ...
  create failed. Degraded-state notices announce on appearance. Nothing announces twice.
...
3. The identity header is already painted from the FR-2 cache: `holocene`, the clone path,
   ...
   one line, `Reading the Board.` The create box does not wait for it.
4. He types `systems table row renders at 0 height`. Nothing else. No description, no state,
   no label — description and target state are optional and the Board's default entry state
   is used (FR-13).
...
6. `Enter`.
7. **Climax.** The row appears at the top of the default entry state's group — the same
   group whose header is the only one expanded — with its human key filled in by Plane. The
   title field is empty again with the caret still in it. **Nothing navigated.** Plane did
   not open, no confirmation appeared, no "view Ticket" beckoned. The thought is out of his
   head and onto the Board, and the panel has given him no reason to still be looking at it.
8. He collapses the Cockpit and goes back to the page. Elapsed: under fifteen seconds.
...
2. He types `what's in progress?`. The classification control reads **Streamed Exchange**,
   with one line under it: `This will stream back here.` The context chip shows the page
   title; focusing it reveals the URL (FR-10).
...
6. He reads it, then types `start on the resolver ticket`. The classification control flips
   itself to **Dispatched Command**, with the line
   `This will be published and acknowledged. The result arrives later.` He does not flip it
   back. The classifier biases toward dispatch when uncertain (FR-6) and this one is not
   uncertain.
...
8. **Climax.** He collapses the Cockpit and goes and does something else. The panel document
   is destroyed; the stream is gone; nothing is listening. Twenty minutes later he reopens
...
9. He reads it and collapses the panel again.
...
5. Below it, the exact provisioning command, rendered verbatim as the Bridge returned it, in
   mono, complete, with a copy control.
6. **Climax.** He copies the command, then drags a selection across the clone path in the
   header and copies that too, and pastes both into a terminal. **He never looked anything
   up.** The one thing the Cockpit could not do for him, it handed him in a form he could
   paste — and the reason chat is unavailable and the fix for it are the same sentence.
   v1 states the command; it does not run it (FR-5).
...
alone says `The Bridge is up. Plane is failing.` and the switch marks Tickets degraded while
he is looking at Chat. This is the exact boundary Rule 2 draws.
...
1. `Alt+Shift+A`. The select tool arms. The in-page indicator says so, and the Cockpit —
   ...
   panel time, which is why beats 1–7 cost SM-C1 nothing.
...
5. He reaches the guy at the bottom. A click will not express this. He **drags** — same
   armed mode, no mode switch — and freehand-marks over the figure. This payload is
   **image + relative coordinates**. Relative, because `captureVisibleTab` returns physical
   pixels on a HiDPI display, and relative coordinates are what survive that mismatch. The
   capture is viewport-only and rate-limited to two calls per second, which is ample for a
   human drawing.
6. The extension icon now carries a count — of **his own uncommitted work**, the only kind of
   count this product will ever render.
7. He opens the Cockpit. The pane switch has a third item: **Annotations**, present because
   the batch is non-empty. Four rows. Hovering a row outlines its element in the page; the
   list and the overlay are two views of one state, never two states to reconcile (Figma's
   one genuinely correct decision here). No filters. No search. No status. No sort.
...
9. **Climax.** One control, two destinations. He picks *a Ticket per piece of feedback*:
   ...
   Agent.
10. He collapses the panel. The page is clean.
...
`chrome.storage.local` keyed by (pjid, page URL), not in the page and not in the Cockpit
document. `[ASSUMPTION: this answers the open question of where annotation state lives. Not
the Cockpit — the document is destroyed on every collapse. Not the page — a reload eats it,
which is the single worst failure for a working-memory externalization tool (A7). Not the
Bridge in v2's first cut — a network round trip per annotation is friction on the one loop
that exists to remove friction. Discharge is the only thing that crosses the network.]`
...
- A **redundant selector set**, below.
- For the freehand kind: the image plus **relative** coordinates.

Because the context is captured at capture time, **a lost anchor degrades an annotation; it
never destroys one.** That single decision is what makes the failure state in the next
...
### What the operator sees — the three outcomes
...
- **A redeploy under an open session.** Not detectable from the page. The annotation's
  captured `outerHTML` is what lets a human — or the PM — see what it *was*.
...
- **Vercel Toolbar's single armed mode with two payloads.** Verbatim: "Click and drag while
  ...
  **No mode switch between them.** This is exactly Jarad's two payloads, already shipped as
  one gesture vocabulary, and it is why `[v2]` has one armed mode and not two tools.
...
- **Linear's drafts that survive `Esc` and persist across clients.** Taken as: every draft in
  this product — composer, Ticket title, comment bubble — is debounced to
  `chrome.storage.local`, because the Cockpit document is destroyed on every collapse and
  "interruption destroys the capture" is fatal to a working-memory tool.
...
- **GitHub's `Y` permalink.** The only prior art anywhere in the sweep for *deliberately
  ...
  capturing `outerHTML` and a text quote alongside the selector.
...
- **Mandatory screenshots on every item** — Marker.io, Usersnap, Userback anchor to a raster;
  ...
  changes." `[v2]`'s element-anchored kind carries **no image**, by design.
...
- **Losing a comment on navigation or reload** — scored in ybug's own friction table as the
  ...
  `chrome.storage.local` keyed by (pjid, page URL).
...
|---|---|---|
| **Tab switch** | The panel document **survives**. DOM, JS state and open sockets persist; the service worker messages the long-lived document to re-render (addendum §C) | FR-1's re-evaluation is an in-place re-render keyed by the new `(pjid, generation)`, not a reload. Focus lands on the pane switch. Chat state survives with no persistence work |
...
| **Panel close** | Same as collapse | Same |
...
2. **Visual identity.** Native to Chrome (kapture's DevTools-dark precedent) or native to
   33GOD? This has been open since the 2026-08-25 DeloHQ run died on the same question.
...
5. **Is the freehand image load-bearing for the PM, or was it shorthand for "it has to look
   ...
   `activeTab` and no rate limit, and produces no image.
6. **Do cross-origin iframes and shadow-DOM elements need to be pickable?** `querySelector`
...
7. **PRD §12 Q4 — what emits the `pjid` declaration into served pages?** Not a design
