...
**Consequences (testable):**
- First token renders within the §5 budget.
...
- It binds to the tailnet interface — **not** loopback, which would be unreachable from the laptop, and **not** `0.0.0.0`.
...
7. **Are extension-context requests to a tailnet address subject to Private Network Access preflight enforcement?** Undocumented, and less certain than the loopback case because `100.64.0.0/10` sits awkwardly in PNA's address-space taxonomy. The §5 mitigations are cheap and unconditional so this does not block, but it wants verifying against current stable Chrome before the transport is locked, and re-verifying on version bumps. **Verify empirically; do not resolve from documentation.**
8. **Does serving the Bridge over HTTPS with a real certificate remove the PNA problem entirely?** Tailscale can issue certs for a MagicDNS name. If that sidesteps Q7, it is worth doing for that reason alone.
