...
7. **OPEN — and the test machine is `carries-macbook-air` (macOS).** Run it there and nowhere else; a result from another node does not transfer, and Chrome versions on this tailnet span 151–155. Note Chrome 153+ ships **Local Network Access**, which gates on the address-space transition with a permission prompt rather than on secure context — so HTTPS likely removes the PNA class but may **not** remove the LNA class. Do not treat `tailscale serve` plus TLS as having closed this. **Are extension-context requests to a tailnet address subject to Private Network Access preflight enforcement?** Undocumented, and less certain than the loopback case because `100.64.0.0/10` sits awkwardly in PNA's address-space taxonomy. The §5 mitigations are cheap and unconditional so this does not block, but it wants verifying against current stable Chrome before the transport is locked, and re-verifying on version bumps. **Verify empirically; do not resolve from documentation.**
