...
- addressed_findings:
  - `[low]` `[patch]` A request whose client left during the body read still fetched the registry and minted. `mutatingRoute` now checks the signal before `resolve` as well as after it.
  - `[low]` `[patch]` A client dropping a mutating request was logged as an error `handler_failed` (DS-5). An incomplete request's `'error'` ("aborted") or `'close'`, and both of the guard's abort checks, now throw a module-private `RequestAborted`. `createBridgeServer` answers nothing and logs no DS-5 line for it; the `'close'` request log still records the request.
  - `[low]` `[patch]` The abort-on-client-disconnect path had no test. `http.test.ts` now drops a raw socket while a 100ms resolver is pending, inside the 200ms deadline, and asserts the handler never runs. A second test drops mid-body and asserts no resolve and no `handler_failed` line.
  - `[low]` `[patch]` The `mutation_rejected` test checked only the last line. It now asserts one line per 400 case, each with its own error code.
...
**Summary:** A follow-up review pass over Story 1.7. The Story 1.7 behavior stands as reviewed: `assertCurrentGeneration` in `registry/generation.ts` is the one definition of stale. `mutatingRoute` validates the body `generation`, re-resolves the pjid fresh, and refuses stale with `409 stale_generation` before its handler runs. `createBridgeServer` refuses any unguarded mutating method at construction. This pass fixed how the guard handles a client that disconnects.
- The guard now checks the abort signal before the registry fetch as well as after it.
- A client that disconnects is no longer logged as a DS-5 internal error.

**Files changed in this pass:**
- `packages/bridge/src/server/http.ts`: `RequestAborted`. `readBody` maps an incomplete request's `'error'` and `'close'` to it. The guard checks the signal before `resolve`. The server answers and logs nothing for `RequestAborted`.
- `packages/bridge/src/server/http.test.ts`: a `/v1/gone/:pjid` fixture, two raw-socket disconnect tests, and a per-case `mutation_rejected` assertion.

**Review findings:** 4 patches applied (all low), 0 deferred, 22 rejected. The rejected findings include:
- Findings the spec or the previous pass already settles: `200 {degraded}` for DS-2/6/7/25, no `CONTRACT_VERSION` bump, DS-5 for ahead, no `EXPERIENCE.md` rows, draining oversized bodies, the DS-25 fallback without params, the `http.ts` → `registry/generation.ts` import, and DS-25 answered at the resolver.
- A deadline firing mid-handler. There is no capability yet, and handlers get `ctx.signal`.
- The resolver signature taking no signal.
- The ahead log lost when it lands after the deadline.
- The duplicate `generation < 1` check.
- Cosmetic typing and log fields: `HttpRefusal` status, optional `signal` or `pjid`, `field` missing from the log.
- A `mutatingRoute` handler registered under `GET`.
- No production mutating route yet. That is the spec's own "Never".

**Follow-up review recommendation:** false. Patched: high 0, medium 0, low 4. Score is 3×0 + 4 = 4, which is below 5.

**Verification:**
- `mise run lint && mise run test && mise run build`: exit 0. Contract passed 5/5; bridge passed 138/138, with 0 skipped. All `mutating.conformance.test.ts` cases pass.
- The new mid-body disconnect test failed before the `'error'` mapping, with `handler_failed` detail `"aborted"`. It passes after.
- `grep -rn "received < current\|received > current" packages --include='*.ts'` has its only hits in `registry/generation.ts`.

**Residual risks:**
- There is no production mutating route yet. Epic 2 and Epic 3 must register through `mutatingRoute`.
- A deadline or disconnect that fires after a handler has started does not stop it. Handlers with side effects must honor `ctx.signal`.
- The guard is a pre-check, not a lock. A rename landing between the guard and a later async commit is not re-checked.
- `RequestAborted` is silent apart from the `'close'` request log. A real bug that coincides with a client disconnect on a mutating route would not produce a DS-5 line.
